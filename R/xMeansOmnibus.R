...
estimateEffectOmnibus <- effectMeanOmnibus <- function(x,...) 
  UseMethod("effectMeanOmnibus")

estimateEffectOmnibus.bss <- function(DescStats,conf.level=.90,main=NULL,digits=3) {
  temptab <- .unformatFrame(describeMeansOmnibus(DescStats,main=main,digits=digits)[[1]])
...
estimateEffectOmnibus.wss <- function(DescStats,CorrStats,conf.level=.90,main=NULL,digits=3) {
...
estimateEffectOmnibus.default <- function(...,mu=0,conf.level=.90,rope=NULL,main=NULL,digits=3) {
  DescStats <- .unformatFrame(describeMeans(...)[[1]])
  class(DescStats) <- "wss"
  CorrStats <- .unformatFrame(describeCorrelations(...)[[1]])  
...
estimateEffectOmnibus.formula <- function(formula,mu=0,conf.level=.90,rope=NULL,main=NULL,digits=3) {
  DescStats <- .unformatFrame(describeMeans(formula)[[1]])
  class(DescStats) <- "bss"
