...
estimateSubsets <- function(...,contrast,conf.level=.95,labels=NULL,main=NULL,digits=3) {
  ...
  res1 <- data.matrix(estimateContrast(...,contrast=con1,conf.level=conf.level)[[1]])
  con2 <- ifelse(contrast>0,0,abs(contrast))
  res2 <- data.matrix(estimateContrast(...,contrast=con2,conf.level=conf.level)[[1]])
