...
plotData.default <- function(...,add=FALSE,main=NULL,ylab="Outcome",xlab="",method="jitter",col="gray60",pch=1) {
  data <- data.frame(...)
...
plotData.formula <- function(formula,add=FALSE,main=NULL,ylab=NULL,xlab="",method="jitter",col="gray60",pch=1,...) {
  x <- eval(formula[[3]])
...
addplotData.default <- function(...,method="jitter",col="gray60",pch=1) {
  data <- data.frame(...)
  mx <- ncol(data)+.15
  mn <- 1+.15
...
addplotData.formula <- function(formula,method="jitter",col="gray60",pch=1,...) {
  x <- eval(formula[[3]])
  adjustX <- as.numeric(x)+.15
  mn <- min(adjustX,na.rm=TRUE)
  mx <- max(adjustX,na.rm=TRUE)
