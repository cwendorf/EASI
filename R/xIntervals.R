...
estimateIntervals <- function(x, contrast = NULL, ...) {
  ...
  if (!is.null(contrast)) {estimateMeanSubsets(x, contrast = contrast, ...)}
  else if (howmany == 2) {estimateMeanComparison(x, ...)}
  else {estimateMeans(x, ...)}
}
...
plotIntervals.default <- function(x, contrast = NULL, ...) {
  ...
  if (!is.null(contrast)) {plotMeanSubsets(x, contrast = contrast, ...)}
  else if (howmany == 2) {plotMeanComparison(x, ...)}
  else {plotMeans(x, ...)}
  invisible(eval(x))
}
