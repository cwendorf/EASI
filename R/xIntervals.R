...
.intervalsMain <- function(results,add=FALSE,main=NULL,ylab="Outcome",xlab="",ylim=NULL,line=NULL,rope=NULL,values=TRUE,digits=3,connect=FALSE,pos=2,pch=16,col="black",offset=0,intervals=TRUE,...) {
  ...
  if(intervals) {
    points(seq_len(nrow(results))+offset,results[,1],pch=pch,cex=1.5,col=col)
...
.intervalsMulti <- function(results,main,ylab,xlab,col) {
  ...
  axis(1, seq_len(nrow(results[[1]])), row.names(results[[1]])) 
  for (i in seq_along(results)) {
    if(length(col)==1) {tempcol=col} else {tempcol=col[i]}
    for (j in seq_len(nrow(results[[i]]))) {lines(x=c(j+(i-(length(results)+1)/2)*.15,j+(i-(length(results)+1)/2)*.15),y=c(results[[i]][,4][j],results[[i]][,5][j]),lwd=2,col=tempcol)}
    points(seq_len(nrow(results[[i]]))+(i-(length(results)+1)/2)*.15,results[[i]][,1],cex=1.5,pch=16,bty="l",col=tempcol)
    if(class(results)=="wss") lines(seq_len(nrow(results[[i]]))+(i-(length(results)+1)/2)*.15,results[[i]][,1],bty="l",col=tempcol)
  }
}
