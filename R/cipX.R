...
.cipMain <- function(results,main,ylab,xlab,mu,rope,values,ylim,digits,connect,pch=16,slab=NULL,add=FALSE,points=TRUE) {
  if(!add) {
  ...
  if(points) {
    points(results[,1],pch=pch,cex=1.5)
...
.cipComp <- function(results,main,ylab,xlab,rope,values,ylim,digits,connect,pch=c(16,16,17),slab=NULL,add=FALSE,points=TRUE) {
  graph <- results
  ...
  if(points) {
    points(c(1,2,3),graph[,1],pch=pch,cex=1.5)
